...
void StealMarkingTask::do_it(GCTaskManager* manager, uint which) {
  ...
  } while (!terminator()->offer_termination(_steal_option, 0));
}
...
void StealRegionCompactionTask::do_it(GCTaskManager* manager, uint which) {
  ...
  while(true) {
    if (_steal_option && ParCompactionManager::steal(which, &random_seed, region_index)) {
      PSParallelCompact::fill_and_update_region(cm, region_index);
      cm->drain_region_stacks();
    } else {
      if (terminator()->offer_termination(_steal_option, 0)) {
        break;
      }
      // Go around again.
    _steal_option = false;
