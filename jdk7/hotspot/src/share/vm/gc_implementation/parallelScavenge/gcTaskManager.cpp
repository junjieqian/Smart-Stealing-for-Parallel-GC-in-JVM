...
void GCTaskManager::initialize() {
  ...
  _root_flag = NEW_C_HEAP_ARRAY(bool, workers());
  for (uint w = 0; w < workers(); w += 1) { 
    set_root_flag(w, false);
  }
}
...
GCTask* GCTaskManager::get_task(uint which) {
  ...
  // if the task is steal-task, determine whether should do steal work
  if (! result->is_steal()) {
    increment_busy_root_workers();
    set_root_flag(which, true);
  }
  else {
    // decide whether it should steal, then which to steal
    //if (busy_root_workers() < busy_workers() - 2) {
    if (busy_root_workers() < 1) {
      result->set_steal(false);
      //tty->print_cr("uid %d, busy root workers %d, busy workers %d, set false", which, busy_root_workers(), busy_workers());
    }
    //else tty->print_cr("uid %d do the stealing work", which);
    ++_steal_count;
  }
  return result;
  // Release monitor().
...
void GCTaskManager::note_completion(uint which) {
  ...
  // set back the root worker flag
  if (root_flag(which)) {
    decrement_busy_root_workers();
    set_root_flag(which, false);
  }
...
uint GCTaskManager::increment_busy_root_workers() {
  assert(queue()->own_lock(), "don't own the lock");
  _busy_root_workers += 1;
  return _busy_root_workers;
}

uint GCTaskManager::decrement_busy_root_workers() {
  assert(queue()->own_lock(), "don't own the lock");
  _busy_root_workers -= 1;
  return _busy_root_workers;
}
...
bool GCTaskManager::root_flag(uint which) {
  assert(which < workers(), "index out of bounds");
  return _root_flag[which];
}

void GCTaskManager::set_root_flag(uint which, bool value) {
  assert(which < workers(), "index out of bounds");
  _root_flag[which] = value;
}
