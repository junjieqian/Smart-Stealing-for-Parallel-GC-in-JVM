...
public:
  ...
  // Abstract is_steal task method
  virtual bool is_steal() {
	  return false;
  }

  virtual void set_steal(bool steal_option) {
...
private:
  ...
  uint                      _steal_count;       // Count of popped-out steal tasks.
  uint                      _busy_root_workers; // Number of current busy workers on non-stealing tasks
  bool*                     _root_flag;         // Array of flag per root worker.
public:
  // Factory create and destroy methods.
...
protected:
  ...
  bool root_flag(uint which);
  void set_root_flag(uint which, bool value);
  ...
  void reset_busy_workers() {
    _busy_workers = 0;
    _busy_root_workers = 0;
  }
  uint increment_busy_root_workers();
  uint decrement_busy_root_workers();
