...
private:
  ...
  // Watermark of the task queue
  uint _watermark_queue;
  // Last stealing choice
  uint _last_steal;
  // timestamp of the task queue
  TimeStamp timer;
  jlong _timestamp_queue;
  // finishes the first iteration of push
  bool _start_pop;
...
public:
  ...
  // set/reset the watermark of the queue
  uint get_watermark() { return _watermark_queue; }
  void reset_watermark() { _watermark_queue = -1; }
  // Set/reset the last steal target
  uint get_laststeal() { return _last_steal; }
  void reset_laststeal() { _last_steal = -1; }
  void set_laststeal(uint k) { _last_steal = k; }

  // set/reset the timestamp of the queue started
  jlong get_timestamp() { return _timestamp_queue; }
  void reset_timestamp() { _timestamp_queue = 0; }
...
public:
  ...
  GenericTaskQueueSet(int n) : _n(n) {
    ...
    _steal_sign = NEW_C_HEAP_ARRAY(bool, n);
    for (int i = 0; i < n; i++) {
      _queues[i] = NULL;
    }
    for (int i = 0; i < n; i++) {
...
GenericTaskQueueSet<T>::steal(uint queue_num, int* seed, E& t) {
  // Consider, 1. watermark size(already exclude queue_size == 1); 
  // 2. remaining queue size
  // 3. one task queue cannot be stolen multiple times
  // 4. how long the queue has been worked on
  /*
  if (_n > 2) {
    int best_i;
    uint best_wt = 0;
    uint best_sz = 0;
    for (int i = 0; i < _n; i++) {
      if (i == queue_num) continue;
      uint wt = _queues[i]->get_watermark();
      uint sz = _queues[i]->size();
      if (_steal_sign[i]) {
        _steal_sign[i] = false;
        continue;
      }
        ...
        if ( wt == 0 && sz > 1) {
          // still pushing, no popping yet
          best_sz = sz;
          best_i = i;
          _steal_sign[i] = true;
          break;
        } else if (best_wt < wt && best_sz < sz) {
          best_wt = wt;
          best_sz = sz;
          best_i = i;
        }
    ...
    return best_sz > 0 && _queues[best_i]->pop_global(t);
  } else if (_n == 2) {
    int i = (queue_num + 1) % 2;
    return _queues[i]->pop_global(t);
  } */
  uint last_steal = queue(queue_num)->get_laststeal();
  if (last_steal >= 0 && last_steal != queue_num && last_steal < _n) {
    if (_queues[last_steal]->size() > 1) {
      if ( queue(last_steal)->pop_global(t)) {
        TASKQUEUE_STATS_ONLY(queue(queue_num)->stats.record_steal(true));
        return true;
      }
    }
  }
  queue(queue_num)->reset_laststeal();
...
GenericTaskQueueSet<T>::steal_best_of_2(uint queue_num, int* seed, E& t) {
  if (_n > 2) {
    ...
    if (sz2 > sz1) {
      _queues[queue_num]->set_laststeal(k2);
      return _queues[k2]->pop_global(t);
    }
    else {
      _queues[queue_num]->set_laststeal(k1);
...
public:
  ...
  // should_exit_termination() in steal tasks
  bool offer_termination(bool steal_option, int s) {
    _steal_option = steal_option;
    return offer_termination(NULL);
  }
...
GenericTaskQueue<E, N>::pop_local(E& t) {
  ...
  if (size(localBot, tp) > 0) {
    ...
    if (! _start_pop) {
      _watermark_queue = size(localBot, tp);
      timer.update();
      _timestamp_queue = timer.ticks();
      _start_pop = true;
    }
    return true;
  } else {
